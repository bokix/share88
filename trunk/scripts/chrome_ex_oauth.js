...
ChromeExOAuth.prototype.clearTokens = function() {
  ...
  delete localStorage[this.key_token + 'user_id'];
};
...
/**
 * Stores an userid for the configured scope.
 * @param {String} token The token to store.
 */
ChromeExOAuth.prototype.setUserId = function(id) {
  localStorage[this.key_token + 'user_id'] = id;
};

/**
 * Retrieves any userid for the configured scope.
 * @return {String} The stored token.
 */
ChromeExOAuth.prototype.getUserId = function() {
  return localStorage[this.key_token + 'user_id'];
};
